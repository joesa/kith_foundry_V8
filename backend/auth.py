...
# If no JWT secret, we'll validate via Supabase API instead
...
async def _validate_token_via_supabase(token: str) -> dict | None:
    ...
    try:
        # Try with SSL verification first, fall back to unverified if cert path is broken
        # (common in mixed Anaconda+venv environments)
        try:
            import certifi
